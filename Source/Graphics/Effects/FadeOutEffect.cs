...
{
    public class FadeOutEffect : IEffect
    {
        ...
        public FadeOutEffect(float duration, float targetAlpha = 0.0f)
        {
            _id = Guid.NewGuid();
        ...
        public FadeOutEffect(float duration, Guid id, float targetAlpha = 0.0f)
        {
            _id = id;
            _duration = duration > 0 ? duration : 0f;
            _targetAlpha = Mathf.Clamp01(targetAlpha);
        ...
        public void Update(float deltaTime)
        {
            ...
            foreach (var target in Targets.ToList())
            {
                if (target is IHasAlpha alphaTarget)
                {
                    ...
                    {
                        alphaTarget.Alpha = 1f + (_targetAlpha - 1f) * progress;
                    }
                }
            }
        }
